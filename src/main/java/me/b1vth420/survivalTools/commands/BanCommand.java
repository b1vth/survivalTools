...
public class BanCommand extends Command {
    public BanCommand() {
        super("ban", "Banowanie graczy", "ehcTools.ban", "/ban <nick> <powod>", false, 1, new String[0]);
    }
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        Bukkit.broadcastMessage(m.getMessage("playerBannedMessage/u").replace("{PLAYER_NAME}", p.getName()).replace("{REASON}", reason).replace("{ADMIN}", admin));
    }
}